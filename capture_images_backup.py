...
def _gui_wait_enter(timeout_ms=None):  # noqa: ARG001
    ...
    is ignored â€” the GUI's wait is driven by a short self-pump loop.
    """
    if _use_gui():
...
def _gui_pump(timeout_ms=30):  # noqa: ARG001
    ...
    """
    if _use_gui():
        # If a click already happened, stop pumping â€” the caller
...
def _gui_choose_next_step():
    """Show the post-capture menu. Returns one of:
       'calibrate', 'test', 'capture', 'quit'."""
    if _use_gui():
        return _GUI.choose_next_step()
    # Fallback: terminal menu (existing post_capture_menu)
...
def label_single_preview(frame, angle):
    ...
    cv2.putText(frame, "ENTER = capture all   |   'r' = retry   |   'q' = quit",
                (12, h - 14),
        ...
        try:
            ...
            if _use_gui():
                ...
                _gui_status(
                    ...
                    "Both camera windows are now LIVE.\n"
                    "Check the framing in BOTH windows, then press"
                    " the green ENTER button below to capture.")
                _gui_buttons(primary="ENTER  (CAPTURE ALL)",
                             show_retry=True, show_quit=True)
            else:
            ...
            while action_btn is None:
                ...
                # 2) waitKey flushes the imshow draw queue AND reads
                #    the OpenCV-window keyboard. 30ms ~ 33 fps.
            ...
            for entry in streams:
                ...
                stream.stop()
            cv2.destroyAllWindows()
            time.sleep(0.5)  # let AVFoundation settle between passes
...
# ---------------------------------------------------------------------------
def run_calibration():
    """Run the side-points and border-region calibration scripts.

    Each script opens one window per side (front / left / right / back).
    For each side, the user clicks metal points (or border polygon) and
    presses 's' to save. The scripts write:

        models/<side>.json              (per-side)
        models/side_points.json         (legacy merged)
        models/border_regions.json      (legacy merged)
    """
    print("\n" + "=" * 60)
    ...
    if missing:
        ...
        print("Run capture again and press ENTER before picking Calibration.")
        return False
...
# ---------------------------------------------------------------------------
# Post-capture menu (continuous loop)
# ---------------------------------------------------------------------------
def post_capture_menu():
    """
    Keep showing the menu until the user quits. Return values:
        1 -> Calibration Mode (caller should re-enter capture loop)
        2 -> Testing Mode    (caller should re-enter capture loop)
        3 -> Capture another chair (caller should reopen cameras)
        'q' -> Quit
    """
    print("\n" + "=" * 60)
    ...
    print("  2) Testing Mode      (inspect new chair against those JSONs)")
    print("  3) Capture another chair  (reopen cameras)")
    print("  q) Quit")
    print("=" * 60)
...
def post_capture_menu_text():
    """Text-based fallback for the GUI's choose_next_step(). Returns
    one of: 'calibrate', 'test', 'capture', 'quit'."""
...
def wait_for_rotate_button():
    ...
    if _use_gui():
        _gui_status(
            "READY TO ROTATE TO 90°",
            "BACK + FRONT images are captured.\n\n"
            "Press the PHYSICAL ROTATE BUTTON.\n"
            "Waiting for GPIO23..."
        )
        _gui_buttons(primary="WAITING FOR GPIO23...",
                     show_retry=False, show_quit=True)
    ...
    while True:
        ...
        while True:
            ...
            if choice == 'calibrate':
                if _use_gui():
                    _GUI.restore_action_buttons()
                    _gui_status("CALIBRATION MODE",
                                "Running side-points and border-region"
                                " calibration. Follow the on-screen"
                                " instructions in the OpenCV windows.")
                run_calibration()
                # After calibration, hard-exit. The src/*.py scripts loaded
                # their own OpenCV state via subprocess, but the parent
                # process is tainted by the capture step's AVFoundation
                # calls, so we don't trust normal interpreter shutdown.
                if _use_gui():
                    _GUI.close()
                return 0
            ...
            if choice == 'test':
                if _use_gui():
                    _GUI.restore_action_buttons()
                    _gui_status("TESTING MODE",
                                "Running inspection against the"
                                " calibrated models. Watch the OpenCV"
                                " windows for results.")
                run_inspection()
                if _use_gui():
                    _GUI.close()
            ...
            if choice == 'capture':
                # Capture another chair - restart the capture loop.
                if _use_gui():
                    _GUI.restore_action_buttons()
                    _gui_status("STARTING NEW CAPTURE",
                                "Position the next chair and get ready"
                                " for Pass 1.")
                print("\nStarting a new capture pass ...")
                break  # inner loop -> back to capture_one_chair
