...
class CameraStream:
    ...
    def health(self, stale_after=1.5):
        ...
        A camera can be connected and still be unusable (for example, the
        lens cap is fitted).  Therefore health checks both:
          1) fresh frames are arriving, and
          2) the frame is not almost completely black / blocked.
        """
        try:
        ...
        # ----------------------------------------------------------
        # Lens-cap / blocked-view detection
        # ----------------------------------------------------------
        # Use the central 90% of the RAW frame so small black borders do
        ...
        try:
            ...
            std_level = float(np.std(gray))
            dark_ratio = float(np.mean(gray < 20))

            # A covered lens normally gives a very dark, low-detail frame.
            # Requiring all three conditions makes this much less likely to
            # reject a legitimately dark chair/background.
            if (
                mean_level < 18.0
                and std_level < 14.0
                and dark_ratio > 0.92
            ):
                return "VIEW BLOCKED", "error"

            # Also catch a completely washed-out / covered-by-bright-light
            # image because it is not usable for inspection either.
            bright_ratio = float(np.mean(gray > 245))
            if (
                mean_level > 245.0
                and std_level < 10.0
                and bright_ratio > 0.92
            ):
                return "VIEW BLOCKED", "error"

        except Exception:
            # If image-quality analysis itself fails, do not call the
            # camera healthy.
            return "CHECK IMAGE", "error"
        ...
        try:
            ...
            while action_btn is None:
                ...
                if _use_gui():
                    ...
                    fixture_ready = fixture_ready_for_pass(pass_idx)
                    capture_ready = cameras_ready and fixture_ready

                    # Do not allow the operator to capture/advance while
                    # either camera view is blocked/unhealthy or the fixture
                    # is not confirmed at the correct position.
                    _GUI.set_capture_enabled(capture_ready)

                    if not capture_ready:
                        if not cameras_ready:
                            _GUI.set_body(
                                "CAPTURE BLOCKED.\n\n"
                                "Both camera views must be OK. "
                                "Remove any lens cap/obstruction and check "
                                "the camera connection."
                            )
                        elif not fixture_ready:
                            required = "0° HOME" if pass_idx == 0 else "90° POSITION"
                            _GUI.set_body(
                                "CAPTURE BLOCKED.\n\n"
                                f"Fixture must be confirmed at {required}."
                            )
                    else:
                        _GUI.set_body(
                            "Both cameras are LIVE and healthy.\n"
                            "Fixture position is confirmed.\n\n"
                            "Check the framing, then click "
                ...
                if _use_gui() and _GUI._action is not None:
                    ...
                    if pending_action == "enter":
                        ...
                        while len(health_now) < 2:
                            health_now.append(("N/A", "unknown"))

                        cameras_ready_now = (
                            health_now[0][1] == "ok"
                            and health_now[1][1] == "ok"
                        )
                        fixture_ready_now = fixture_ready_for_pass(pass_idx)

                        if not (cameras_ready_now and fixture_ready_now):
                            # Ignore stale/programmatic capture action.
                            action_btn = None
                            continue

                    action_btn = pending_action
                    break
            ...
            if action_btn == 'enter':
                # Last safety/quality gate immediately before image capture.
                final_health = [
                    entry[0].health()
                    for entry in streams[:2]
                ]
                while len(final_health) < 2:
                    final_health.append(("N/A", "unknown"))

                if not (
                    final_health[0][1] == "ok"
                    and final_health[1][1] == "ok"
                    and fixture_ready_for_pass(pass_idx)
                ):
                    print(
                        "  Capture rejected: camera view or fixture "
                        "became NOT READY."
                    )
                    action = "retry"
                    continue
...
def fixture_ready_for_pass(pass_idx):
    """Return True only when fixture feedback matches the requested pass.

    Pass 0 (BACK + FRONT) requires 0° HOME.
    Pass 1 (RIGHT + LEFT) requires 90° POSITION.
    """
    text, state = get_fixture_position_status()
